...
parser.add_argument('--protein', type=str, required=True, help='Protein sequence file')
parser.add_argument('--ligands', type=str, required=True, help='Ligand smies file')
...
ligand_df = pd.read_csv(os.path.join(args.ligands))

protein_seqs = []
with open(args.protein) as fr:
    for line in fr:
        line = line.strip()
        if line:
            protein_seqs.append(line)
print('Initialising protein sequence to Protein Graph')
protein_dict = protein_init(protein_seqs)
# print(protein_dict)
ligand_smiles = ligand_df['Ligand'].unique().tolist()
print(ligand_smiles)
print('Initialising ligand SMILES to Ligand Graph')
ligand_dict = ligand_init(ligand_smiles)
torch.cuda.empty_cache()
## drop any invalid smiles
screen_df = ligand_df[ligand_df['Ligand'].isin(list(ligand_dict.keys()))].reset_index(drop=True)
